  ...
  create: protectedProcedure
    .input(
      z.object({
        ...
        rating: z
          .number()
          .min(1, { message: "Rating must be required" })
          .max(5),
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const exitingReviewData = await ctx.db.find({
        collection: "reviews",
        where: {
          and: [
            {
      ...
      if (exitingReviewData.totalDocs > 0) {
        throw new TRPCError({
          code: "BAD_REQUEST",
  ...
  update: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (existingReview.user !== ctx.session.user.id) {
        throw new TRPCError({
          code: "FORBIDDEN",
          message: "You can not allowed to update this reviews",
        });
      }
